...
byte pull(State_t* i_state)
{
    ...
    uint32 l_value = i_state->m_input[i_state->m_inputPos];
...
byte readCode(HuffmanTree_t* i_huffmanTree, State_t* i_state, uint16* io_code)
{
    ...
    CHECK(needBits(i_state, 32));
    uint16 l_index = 0;
    while (READBITS((*i_state), 32) < i_huffmanTree->m_codeComp[l_index])
    {
        ++l_index;
    }

    byte l_bitsNb = i_huffmanTree->m_codeBits[l_index];
    *io_code = i_huffmanTree->m_codeValue[i_huffmanTree->m_tabOffset[l_index] -
                                          ((READBITS((*i_state), 32) - i_huffmanTree->m_codeComp[l_index]) >> (32 - l_bitsNb))];
    return dropBits(i_state, l_bitsNb);
}

byte buildHuffmanTree(HuffmanTree_t* i_huffmanTree, int16* i_workingBitTab, int16* i_workingCodeTab)
{
    assert(i_huffmanTree != NULL);
    ...
    uint32 l_code = 0;
    byte l_bitsNb = 0;
    uint16 l_codeCompIndex = 0;
    uint16 l_codeOffset = 0;

    while (l_bitsNb < MAX_CODE_BITS_LENGTH)
    {
        if (i_workingBitTab[l_bitsNb] != -1)
        {
            int16 l_currentSymbol = i_workingBitTab[l_bitsNb];
...
byte parseHuffmanTree(State_t* i_state, HuffmanTree_t* i_huffmanTree, HuffmanTree_t* i_huffmanTreeDict)
{
    ...
    CHECK(needBits(i_state, 16));
    uint16 l_numberOfSymbols = (uint16)(READBITS((*i_state), 16));
    CHECK(dropBits(i_state, 16));

    if (l_numberOfSymbols > MAX_CODE_NUMBER)
    {
        printf("Too many symbols to decode: %u\n", l_numberOfSymbols);
        return KO_RES;
    }

    int16 l_workingBitTab[MAX_CODE_BITS_LENGTH];
    int16 l_workingCodeTab[MAX_CODE_NUMBER];

    // Initialize our workingTabs
    memset(&l_workingBitTab, 0xFF, MAX_CODE_BITS_LENGTH * sizeof(int16));
    memset(&l_workingCodeTab, 0xFF, MAX_CODE_NUMBER * sizeof(int16));

    int16 l_remainingSymbols = l_numberOfSymbols - 1;

    // Fetching the code repartition
    while (l_remainingSymbols > -1)
    {
        uint16 l_code = 0;
        CHECK(readCode(i_huffmanTreeDict, i_state, &l_code));

        uint16 l_codeNumberOfBits = l_code & 0x1F;
        uint16 l_codeNumberOfSymbols = (l_code >> 5) + 1;
...
byte fillTabsHelper(byte i_bits, int16 i_symbol, int16* i_workingBitTab, int16* i_workingCodeTab)
{
    assert(i_workingBitTab != NULL);
    assert(i_workingCodeTab != NULL);

    // checking out of bounds
    if (i_bits >= MAX_CODE_BITS_LENGTH || i_symbol >= MAX_CODE_NUMBER)
    {
        return KO_RES;
    }
